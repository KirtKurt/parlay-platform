...
def patch_prospective_repair() -> None:
    path = ROOT / "hello_world" / "mlb_prospective_row_repair.py"
    ...
    premature_exit = '''        if (
            ...
            return result
'''
    if premature_exit in text:
        text = text.replace(premature_exit, "", 1)
    elif "v2-durable-cached-replay" not in text:
        raise RuntimeError("cached terminal early-exit anchor was not found")
    path.write_text(text, encoding="utf-8")


def patch_status_first_reconciliation() -> None:
...
''',
        "mutable incomplete-status error",
    )
    helper = '''
...
'''
    if "def _status_requires_terminal_durability_replay(" not in text:
        text = replace_once(
            text,
...
''',
    ...
    return str(exc) in MUTABLE_INCOMPLETE_STATUS_ERRORS
''' + helper,
            "terminal durability helper",
        )
    replay_gate = '''            lock_evidence = _official_evidence(official_status, slate_date)
            if _status_requires_terminal_durability_replay(official_status):
                raise base.ReconciliationError(
                    "official_status_terminal_durability_incomplete"
                )
'''
    if replay_gate not in text:
        text = replace_once(
            text,
            '''            lock_evidence = _official_evidence(official_status, slate_date)
''',
            replay_gate,
            "status-first durable replay gate",
        )
    path.write_text(text, encoding="utf-8")


def write_regression_tests() -> None:
    path = ROOT / "tests" / "unit" / "test_mlb_terminal_durability_replay.py"
    path.write_text(
        '''from __future__ import annotations
...
def main() -> None:
    patch_prospective_repair()
    patch_status_first_reconciliation()
