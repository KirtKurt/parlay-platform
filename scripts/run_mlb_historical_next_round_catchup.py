...
2026-08-25: trigger the canonical catch-up after the workflow gained an isolated
provider-credential drift repair. No model, gate, or production-authority logic is
changed by this source touch.
"""
...
def summarize_state(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "featureRematerializedSlateCount": _int(state.get("featureRematerializedSlateCount")),
        "featureRematerializationTotalSlateCount": _int(state.get("featureRematerializationTotalSlateCount")),
        "targetSettledGames": target,
        "gamesUntilNextOptimization": max(0, target - eligible),
        "nextOptimizationReady": bool(state.get("nextOptimizationReady") is True or (target > 0 and eligible >= target)),
        ...
        "freshAuditCollectedDayCount": _int(state.get("freshAuditCollectedDayCount")),
        "freshAuditCollectedGameCount": _int(state.get("freshAuditCollectedGameCount")),
...
def _runtime(*, region: str, stack_name: str, expected_handler: str, expected_max_credits: str, template: Path) -> tuple[Any, str, str, dict[str, Any]]:
    ...
    if stack_status not in watchdog.STABLE_STACK_STATUSES:
        raise RuntimeError(f"historical_stack_not_stable:{stack_status}")
    function_name = watchdog._stack_outputs(stack).get("HistoricalOptimizerFunctionName")
    ...
    checks = {
        ...
        "configuredCeiling": environment.get("MLB_HISTORICAL_END_DATE") == expected_ceiling,
        "maxCredits": environment.get("MLB_HISTORICAL_MAX_CREDITS") == str(expected_max_credits),
        "rangeExtensionAuthorized": environment.get("MLB_HISTORICAL_RANGE_EXTENSION_AUTHORIZED") == "true",
    }
    if not all(checks.values()):
        raise RuntimeError(f"historical_runtime_configuration_mismatch:{checks}")
    return lam, function_name, expected_ceiling, {
        "functionName": function_name,
        "stackStatus": stack_status,
        "handler": config.get("Handler"),
        "deployGitSha": environment.get("INQSI_DEPLOY_GIT_SHA"),
        "configuredCeilingDate": expected_ceiling,
        "maximumCredits": environment.get("MLB_HISTORICAL_MAX_CREDITS"),
        "rangeExtensionAuthorized": environment.get("MLB_HISTORICAL_RANGE_EXTENSION_AUTHORIZED"),
        "checks": checks,
    }


def _experiment_advanced(baseline: Mapping[str, Any], current: Mapping[str, Any]) -> bool:
    baseline_summary = summarize_state(baseline)
    current_summary = summarize_state(current)
    return bool(
        current_summary["experimentId"] and current_summary["experimentId"] != baseline_summary["experimentId"]
        or current_summary["optimizationRound"] > baseline_summary["optimizationRound"]
        or (current_summary["optimizationCompletedAtUtc"] and current_summary["optimizationCompletedAtUtc"] != baseline_summary["optimizationCompletedAtUtc"])
    )


def run(*, region: str, stack_name: str, expected_handler: str, expected_max_credits: str, template: Path, max_attempts: int = 4) -> dict[str, Any]:
    lam, function_name, ceiling, runtime_identity = _runtime(region=region, stack_name=stack_name, expected_handler=expected_handler, expected_max_credits=expected_max_credits, template=template)
    baseline_response = watchdog._status(lam, function_name, "next_round_baseline")
    baseline = dict(baseline_response["state"])
    initial_action = classify_state(baseline, expected_ceiling=ceiling)
    ...
    blockers: list[str] = []
    if action not in {WAITING_FOR_EVIDENCE, CHAMPION_WAITING}:
        attempts = max(1, max_attempts) if action == RUN_NEXT_OPTIMIZATION else 1
        previous = baseline
        for attempt in range(1, attempts + 1):
            response = watchdog._invoke(lam, function_name, {"mode": "orchestrate", "run": f"github_next_round_catchup_{attempt}"})
            if response.get("ok") is not True:
                raise RuntimeError(f"historical_optimizer_orchestrate_failed:{response}")
            if response.get("status") == "LEASE_HELD":
                progress.append({"attempt": attempt, "observedAtUtc": datetime.now(timezone.utc).isoformat(), "status": "LEASE_HELD"})
                time.sleep(15)
                continue
            status_response = watchdog._status(lam, function_name, f"next_round_after_{attempt}")
            ...
            if _experiment_advanced(baseline, current):
                new_experiment = True
                break
            if current_action in {WAITING_FOR_EVIDENCE, CHAMPION_WAITING}:
                action = current_action
                break
            time.sleep(5)
        final_action = classify_state(final, expected_ceiling=ceiling)
        if final_action == RUN_NEXT_OPTIMIZATION and not new_experiment:
            blockers.append("next_optimization_did_not_complete_within_bound")
    baseline_summary = summarize_state(baseline)
    ...
    return {
        ...
        "finalAction": classify_state(final, expected_ceiling=ceiling),
        "baseline": baseline_summary,
        "final": final_summary,
        "movement": {
            "eligibleGames": final_summary["eligibleGameCount"] - baseline_summary["eligibleGameCount"],
            "completeSlates": final_summary["completeSlateCount"] - baseline_summary["completeSlateCount"],
            "networkRequests": final_summary["networkRequestCount"] - baseline_summary["networkRequestCount"],
            "optimizationRounds": final_summary["optimizationRound"] - baseline_summary["optimizationRound"],
        },
        "newExperimentCompleted": new_experiment,
        "waitingIsNotFailure": classify_state(final, expected_ceiling=ceiling) in {WAITING_FOR_EVIDENCE, CHAMPION_WAITING},
...
def main() -> int:
    ...
    try:
        result = run(region=args.region, stack_name=args.stack_name, expected_handler=args.expected_handler, expected_max_credits=args.expected_max_credits, template=args.template, max_attempts=args.max_attempts)
