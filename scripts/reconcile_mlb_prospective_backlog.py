...
def _lifecycle_classifications(
    rows: Sequence[Mapping[str, Any]],
) -> List[Dict[str, str]]:
    """Project the settlement-visible game classification authority.

    The protected lock/status boundary proves the provider game identity.
    Canonical settlement intentionally exposes only the official game PK and
    its terminal classification, so the cross-boundary equality check must
    compare exactly those fields rather than inventing an empty identity.
    """
...
def validate_settlement_lock_binding(
    ...
    if _lifecycle_classifications(
        lock_games
    ) != _lifecycle_classifications(settlement_games):
        raise ReconciliationError(
