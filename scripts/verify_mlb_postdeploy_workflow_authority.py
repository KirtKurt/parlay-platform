...
def verify_repository(root: Path = ROOT) -> List[str]:
    ...
    if not deploy:
        ...
        for token in (
            ...
            if token not in deploy_job:
                errors.append(
                    "postdeploy_contract:deploy_output_binding_missing:" + token.strip()
                )
        if not deploy_step:
        ...
        for token in (
            ...
            if token not in dispatch_job:
                errors.append(
                    "postdeploy_contract:dispatcher_binding_missing:" + token.strip()
                )
    ...
    if not postdeploy:
        ...
        for token in (
            ...
            if token not in trigger:
                errors.append(
                    "postdeploy_contract:trigger_or_input_missing:" + token.strip()
                )
        ...
        for token in (
            ...
            "recoveredPostDeployScheduledFailures",
        ):
        ...
        for token in (
            ...
            if token not in publish_job:
                errors.append(
                    "postdeploy_contract:global_publication_missing:" + token.strip()
                )
    ...
    if not publisher:
        ...
        for token in (
            ...
            "proof creation cannot precede SAM completion",
            "PROVENANCE_MARKERS",
            'startswith("sourceDeploy")',
