...
def _http_json(url: str, timeout: int = 8) -> dict[str, Any]:
    request = urllib.request.Request(url, headers={"accept": "application/json", "user-agent": "inqsi-mlb-weather-diagnostic/1.0"})
...
def diagnose_game(game: Mapping[str, Any], *, clock: Callable[[], datetime], fetch_json: Callable[[str, int], dict[str, Any]]) -> dict[str, Any]:
