...
Post-deploy proofs are ordered by the successful canonical SAM deployment,
not by verifier observation time. A retry for one deployment may repair failed
evidence, but it may not replace successful evidence with a failure.
"""
...
def _has_provenance_markers(value: Mapping[str, Any]) -> bool:
    return "deploymentSucceeded" in value or any(
        str(key).startswith("sourceDeploy") for key in value
    )
...
def _sha(value: Any, field: str) -> str:
    parsed = str(value or "")
    if len(parsed) != 40 or any(c not in "0123456789abcdef" for c in parsed):
...
def validate_provenance(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if created_at < sam_completed_at:
        raise ValueError("proof creation cannot precede SAM completion")
...
def selection_reason(
    ...
    try:
        ...
        if _has_provenance_markers(current):
            raise
        legacy = _legacy_metadata(current)
        same_commit = candidate_meta["deployed_commit"] == legacy["deployed_commit"]
        ...
        if same_commit and legacy["ok"] and not candidate_meta["ok"]:
...
def publish(
    ...
    elif current is not None and output_path.resolve() != existing_path.resolve():
