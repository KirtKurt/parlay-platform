...
def reconcile(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    ...
    for repaired_slate, repaired_proof in repaired.items():
        ...
        if (
            ...
            or not isinstance(official_lifecycle_games, list)
            or not isinstance(settlement_lifecycle_games, list)
            or receipt_lifecycle_games != official_lifecycle_games
            or base._lifecycle_classifications(
                receipt_lifecycle_games
            )
            != base._lifecycle_classifications(
                settlement_lifecycle_games
            )
