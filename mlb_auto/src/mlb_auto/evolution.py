...
def _effect(rows, labels, name: str) -> float:
    ...
    return abs(mean(pos)-mean(neg))/math.sqrt(max(pooled,1e-12))
...
def augment_interactions(rows: Sequence[Mapping[str, Any]], names: Sequence[str], max_base: int=8) -> tuple[list[dict],list[str]]:
    base=list(names)
    ...
    for row in rows:
        ...
        for i,a in enumerate(strongest):
            av=_numeric(row.get(a)) or 0.0
            for b in strongest[i+1:]:
                r[f'{a}__x__{b}']=av*(_numeric(row.get(b)) or 0.0)
        out.append(r)
...
def discover_challenger(rows: Sequence[Mapping[str,Any]], labels: Sequence[int], *, min_train: int=50, min_validation: int=20) -> ChallengerResult:
    ...
    # Search remains broad enough to alter model structure, while bounded enough for Lambda autonomy.
    sizes=sorted(set(min(n,len(ranked)) for n in (8,16,32) if min(n,len(ranked))>0))
    configs=[]
    for n in sizes:
        names=ranked[:n]
        configs.append(('linear',names))
        _,aug_names=augment_interactions(tr,names,max_base=min(8,len(names)))
        configs.append(('interaction',aug_names))

    # Corpus grows over time; use fewer epochs for larger cohorts, with model-level early stopping as a second bound.
    epochs=max(100,min(260,int(70000/max(1,len(tr)))))
    learning_rates=(.015,.04)
    penalties=(.0003,.0015)
    best=None
    evaluated=0
    for family,names in configs:
        ...
        if family=='interaction':
            base_names=[n for n in names if '__' not in n]
            train_rows,_=augment_interactions(tr,base_names,max_base=8)
            val_rows,_=augment_interactions(va,base_names,max_base=8)
        for lr in learning_rates:
            for l2 in penalties:
                model=train_logistic(train_rows,ty,feature_names=names,epochs=epochs,lr=lr,l2=l2,
                    metadata={'autonomous':True,'family':family,'lr':lr,'l2':l2,'search_epochs':epochs})
    ...
    return ChallengerResult(model,metrics,names,{
        'candidateRawFeatures':len(candidate_numeric_features(tr)),
        'rankedFeatures':ranked,
        'selectedFeatureCount':len(names),
        'selectedFamily':family,
        'learningRate':lr,
        'l2':l2,
        'trainingRows':len(tr),
        'validationRows':len(va),
        'epochsPerCandidate':epochs,
        'candidateModelsEvaluated':evaluated,
        'searchPolicy':'bounded_adaptive_grid_v2',
    })
