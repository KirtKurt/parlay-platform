...
Model selection is configuration-driven and uses the first invokable model in the ordered MLB Auto fallback list. Configured Claude inference-profile IDs are attempted through Bedrock Runtime first; the Anthropic-compatible Bedrock Mantle endpoint is retained as a secondary endpoint with explicit model-ID translation. The runtime does not require unavailable models, enroll the AWS account in model products, or report a model as verified until a real research invocation succeeds.
