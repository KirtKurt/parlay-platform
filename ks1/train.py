...
def save_artifact(s3, bucket, output):
    ...
    print(json.dumps({'artifact': f's3://{bucket}/{prefix}model.txt' if 'model.txt' in hashes else None,
                      'artifact_prefix': f's3://{bucket}/{prefix}', 'readback_verified': True,
                      'deployed': False, 'registered_for_serving': False,
                      'run_summary': artifact_summary(output),
                      'model_receipts': [item for item in receipts
