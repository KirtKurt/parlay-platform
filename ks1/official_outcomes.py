...
METHOD = 'official_pitch_attribution_and_mound_visit_v9'
...
def needed_games(raw, method=METHOD):
    credit_games = {pk for pk, ab in unfinished_at_bats(raw)} if method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD, CREDIT_METHOD) else set()
    if method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD):
        from ks1.official_pitch_attribution import candidate_groups
    ...
    return sorted(credit_games | {positive_id(row['game_pk']) for row in raw['rows']
                   ...
                   and ((method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD, CREDIT_METHOD, DENOMINATOR_METHOD) and denominator_change(row) is not None)
...
def reconciled_rows(raw, evidence, scheduled_by_game=None, method=METHOD):
    if method not in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD, CREDIT_METHOD, DENOMINATOR_METHOD, LEGACY_METHOD):
    ...
    for pk, source in evidence.items():
        official_index(source, pk, raw['date'],
                       ...
                       pitch_evidence=method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD) and source['receipt']['endpoint'] ==
                       endpoint(pk, pitch_evidence=True),
                       accounting_evidence=method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD) and source['receipt']['endpoint'] ==
                       endpoint(pk, accounting_evidence=True),
                       inning_evidence=method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD) and source['receipt']['endpoint'] ==
    ...
    for index, row in enumerate(rows):
        ...
        if fields:
            ...
            if method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD, CREDIT_METHOD, DENOMINATOR_METHOD):
                change.update(original_fields=prior, derivation_kind='official_pa_denominator')
            changes.append(change)
    if method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD, CREDIT_METHOD):
        for (pk, ab), indices in unfinished_at_bats(raw).items():
            ...
            inning_ending = (method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD) and source['receipt']['endpoint'] == endpoint(pk, inning_evidence=True)
                             and verified_inning_ending(play, source, [raw['rows'][i] for i in indices]))
            walkoff = (method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD) and event not in NON_PA_AT_BAT_END_EVENTS
    ...
    if method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD):
        ...
        changes.extend(derive(rows, evidence, scheduled_by_game, extended_substitutions=method in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD),
                              two_strike_strikeouts=method in (METHOD, TWO_STRIKE_METHOD),
                              prefix_mound_visits=method == METHOD, raw_rows=raw['rows']))
    return rows, changes
...
def verify_reconciliation(payload, completed_by_game, scheduled_by_game=None):
    ...
    if (proof['method'] not in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD, CREDIT_METHOD, DENOMINATOR_METHOD, LEGACY_METHOD) or raw['date'] != payload['date']
...
def verified_batter_credits(payload, scheduled_by_game=None):
    ...
    if proof.get('method') not in (METHOD, TWO_STRIKE_METHOD, INNING_METHOD, SUBSTITUTION_METHOD, ACCOUNTING_METHOD, PITCH_METHOD) or not any(
