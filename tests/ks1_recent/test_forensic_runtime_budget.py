...
@pytest.mark.parametrize('elapsed,remaining', [(0, 8400), (60, 8340), (4800, 3600), (8399, 1)])
...
@pytest.mark.parametrize('elapsed', [8400, 9000, 100000])
...
def test_budget_cli_returns_seconds_or_nonzero_without_a_command(monkeypatch, capsys):
    ...
    assert capsys.readouterr().out.strip() == '7800'
    monkeypatch.setattr(runtime, 'time', lambda: 1008400)
...
def test_workflow_reserves_cleanup_without_extending_timeout_or_hiding_failure():
    ...
    assert runtime.WORK_BUDGET_SECONDS == 140 * 60
