...
def test_pooled_bullpen_regime_advantage_requires_all_four_admitted_parents():
    rows = []
    ...
    assert "forensic_bullpen_fip_regime_advantage" in admitted
    assert "forensic_bullpen_era_regime_advantage" in admitted

    missing_parent = [name for name in raw if name != "home_bullpen_context_fip_30d"]
    admitted2, rejected2, _ = admit(frame, missing_parent, minimum_nonmissing=5)
    feature = "forensic_bullpen_fip_regime_advantage"
