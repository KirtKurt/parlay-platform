...
def test_mantle_catalog_comes_from_mantle_models_endpoint(monkeypatch) -> None:
    _reset(monkeypatch)
    calls = []
...
def test_runtime_catalog_excludes_mantle_only_and_marketplace_providers(
    monkeypatch,
) -> None:
    _reset(monkeypatch)
    ...
    assert "anthropic.claude-3-haiku-20240307-v1:0" not in models
...
def test_configured_models_prefers_endpoint_native_mantle_catalog(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        gateway,
        "runtime_models",
        lambda: ["openai.gpt-oss-20b-1:0", "amazon.nova-lite-v1:0"],
    )
    monkeypatch.setenv(
