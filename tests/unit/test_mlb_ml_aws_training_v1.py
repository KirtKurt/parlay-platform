...
def test_r4_cutoff_rejects_july20_and_every_pre_boundary_lock():
    ...
    assert manifest_value["releaseCutoffUtc"] == "2026-08-02T04:00:00+00:00"
    row = {
...
def test_status_is_read_only_and_attests_deployment_and_manual_first_policy():
    ...
    assert result["releaseCutoffUtc"] == "2026-08-02T04:00:00+00:00"
...
def test_partial_prior_et_slate_cannot_freeze_or_deadlock_manifest(monkeypatch):
    ...
    row = {
        ...
        "commenceTime": "2026-08-02T23:00:00+00:00",
        "featureSnapshot": {"fingerprint": "partial-fingerprint"},
    }
    state = {
        "report": {
            "ok": False,
            "requestedSlateDates": ["2026-08-02"],
            "finalizedSlateDates": [],
    ...
    state["report"] = {
        "ok": True,
        "requestedSlateDates": ["2026-08-02"],
        "finalizedSlateDates": ["2026-08-02"],
        "rows": [copy.deepcopy(row)],
        "slates": [
            {
                "slateDateEt": "2026-08-02",
    ...
    assert store.manifest["assignedSlateDates"]["2026-08-02"]["partition"] == "train"
