...
def test_r4_cutoff_rejects_july20_and_every_pre_boundary_lock():
    ...
    assert manifest_value["releaseCutoffUtc"] == "2026-07-29T04:00:00+00:00"
    row = {
...
@pytest.mark.parametrize(
    ...
    (
        ...
        datetime(2026, 7, 29, 4, 0, 0, 1, tzinfo=timezone.utc),
    ),
)
...
def test_status_is_read_only_and_attests_deployment_and_manual_first_policy():
    ...
    assert result["releaseCutoffUtc"] == "2026-07-29T04:00:00+00:00"
...
def test_partial_prior_et_slate_cannot_freeze_or_deadlock_manifest(monkeypatch):
    ...
    after_midnight_et = datetime(2026, 7, 30, 4, 30, tzinfo=timezone.utc)
    row = {
        "gameId": "mlb_statsapi:partial-prior-date",
        "slateDateEt": "2026-07-29",
        "slateFinalized": False,
        "commenceTime": "2026-07-29T23:00:00+00:00",
        "featureSnapshot": {"fingerprint": "partial-fingerprint"},
    }
    state = {
        "report": {
            "ok": False,
            "requestedSlateDates": ["2026-07-29"],
            "finalizedSlateDates": [],
    ...
    state["report"] = {
        "ok": True,
        "requestedSlateDates": ["2026-07-29"],
        "finalizedSlateDates": ["2026-07-29"],
        "rows": [copy.deepcopy(row)],
        "slates": [
            {
                "slateDateEt": "2026-07-29",
    ...
    assert store.manifest["assignedSlateDates"]["2026-07-29"]["partition"] == "train"
