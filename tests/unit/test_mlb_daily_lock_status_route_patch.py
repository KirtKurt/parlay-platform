...
def test_non_status_lock_execution_preserves_full_diagnostics():
    module, calls = _build_module()
    ...
    response = module.handle(
        {
            "httpMethod": "POST",
            "path": "/api/mlb/lock",
            "body": json.dumps({"date": SLATE, "force": False}),
        },
        None,
    )
    body = json.loads(response["body"])
    ...
    assert calls["diagnostics"] == 0
    assert route_patch._INCLUDE_ATTEMPT_DIAGNOSTICS.get() is True
...
def test_runtime_startup_installs_bounded_lock_status_route():
    source = (HELLO_WORLD / "usercustomize.py").read_text(encoding="utf-8")

    assert "import mlb_daily_lock_status_route_patch" in source
    assert "mlb_daily_lock_status_route_patch.apply" in source
