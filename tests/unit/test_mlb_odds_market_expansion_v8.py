...
def test_featured_historical_url_contains_all_high_value_markets():
    ...
    assert "regions=us%2Cus2" in url
...
def test_cycle_budget_blocks_expensive_event_plan():
    value = v8.enforce_cycle_budget(event_count=8, event_market_count=40, config=cfg(max_estimated_credits_per_cycle=500))
    assert value["estimatedCredits"] == 646
