...
def run(
    *,
    ...
    evidence = evidence_path.read_text(encoding="utf-8", errors="replace")[-28000:]
    ...
    for attempt in range(1, max(1, max_attempts) + 1):
        ...
        if response.get("ok") is not True or response.get("mode") != "engineering_plan":
            ...
            rejected.append({"title": "<lambda-failure>", "reason": reason[:1000]})
            continue
        ...
        if not text:
            ...
            summary["rejected"] = reason
            attempts.append(summary)
            rejected.append({"title": "<empty>", "reason": reason})
            continue
        ...
        try:
            ...
            summary.update({"title": proposed_title, "rejected": reason})
            attempts.append(summary)
            rejected.append({"title": proposed_title, "reason": reason[:1000]})
            continue
        ...
        summary.update({"title": task["title"], "accepted": True})
        attempts.append(summary)
