...
Status at preparation: **not deployed**. No live endpoint, production task identity or end-to-end receipt has been observed. The first AWS STS attempt returned: “network approval was cancelled before a decision was returned.” A subsequent local check with EC2 metadata discovery disabled confirmed “Unable to locate credentials.” No alternate AWS route was used. Required external references are described in [deployment configuration](README.md); do not invent them or equate local tests with deployment success.
