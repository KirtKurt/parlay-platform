...
Before updating an existing stack, deployment captures its original template, parameter values, image/source outputs and service counts. The previous stack must advertise the compatible `RuntimeStateVersion` (`isolated-console-v3`); an older or unknown storage format requires a separately reviewed migration before any update. Keep this marker unchanged only while durable job, transport and publication formats remain backward compatible.

If live verification fails after CloudFormation succeeds, deployment automatically sets the three Console services to zero, waits for their shutdown, stops remaining Console coding/probe tasks and confirms an empty active inventory. It then restores the recorded template/parameters, service counts, image/source and retained filesystem, requiring exact running task/definition/digest evidence. With no previous deployment, the candidate stays stopped and its data is retained. A failed restoration triggers another shutdown attempt and records any unconfirmed stop; it never records deployment as verified. Deployment artifacts preserve the snapshot and rollback outcome. The existing deployment role needs Console-scoped CloudFormation `GetTemplate` and ECS `DescribeServices`/`UpdateService` permissions in addition to its existing deploy/list/describe/stop permissions.

Each coding execution persists its launched task-definition ARN and immutable image before RunTask. Recovery validates that recorded definition and the observed task image, even across controller upgrades. Executions without that provenance remain blocked for explicit reconciliation; cancellation can still stop their verified launch identity. Lost launch responses are discovered across both RUNNING and STOPPED inventories. Do not run older JobStore writers alongside this version. Cancellation stays pending until task/PR closure is confirmed. A verified merge after accepted cancellation is a durable reconciliation conflict.
