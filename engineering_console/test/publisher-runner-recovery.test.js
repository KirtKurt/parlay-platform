...
import { cancelPublication, beginPublicationMerge, arbitrateVerifiedMerge } from '../src/publication-decision.js';
...
test('publisher validates landed merge before durable merged receipt', () => {
  ...
  const record = source.indexOf('record(completion)', finish);
...
test('unconfirmed cancellation retains a pending state and its confirmation error', (t) => {
  ...
  assert.equal(store.get(job.id).status, 'awaiting_publication');
  assert.equal(store.get(job.id).publicationState, 'cancellation_pending');
