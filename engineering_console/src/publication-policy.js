...
// Deliberately narrow first activation. Broader engineering scopes require a
// separately reviewed policy and matching immutable, secret-free checks.
...
export function validatePublisherRequest(manifest, patch, policy) {
  ...
  // The publisher's deployment policy is authoritative; the worker cannot select
  // an easier check or an empty policy in its writable outbox.
