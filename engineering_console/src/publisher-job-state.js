...
export function updateJobFromPublisher(dataDir, receipt) {
  ...
  if (receipt.state === 'merge_conflict') {
    ...
    job.cancelRequested = true;
    job.error = null;
  } else if (receipt.state === 'cancellation_pending' || publicationDecision(dataDir, receipt.jobId) === 'cancelled') {
    job.status = 'awaiting_publication';
    job.publicationState = 'cancellation_pending';
    job.cancelRequested = true;
    job.error = receipt.reason || 'publication_cancellation_unconfirmed';
