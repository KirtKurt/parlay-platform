...
const EXECUTION_FINISHED = new Set(['completed', 'failed', 'cancelled', 'blocked', 'awaiting_publication', 'published', 'awaiting_approval']);
...
export class JobStore {
  ...
  create(input, owner, revision) {
    if (typeof input?.instruction !== 'string' || !input.instruction.trim()) {
      throw Object.assign(new Error('valid_instruction_required'), { status: 400, code: 'EINVAL' });
    }
  ...
  #releaseInstructionIfFinished(id, status) {
    if (EXECUTION_FINISHED.has(status)) this.runtimeInstructions.delete(id);
  }

  #applyCommitted(job, target) {
    const persisted = JSON.parse(fs.readFileSync(target, 'utf8'));
    this.#releaseInstructionIfFinished(job.id, persisted.status);
    const runtimeInstruction = this.runtimeInstructions.get(job.id);
  ...
  save(job) {
    ...
    if (result.error || result.status !== 0) throw this.#writeError(result.status);
  ...
  get(id) {
    ...
    try {
      ...
      this.#releaseInstructionIfFinished(id, job.status);
