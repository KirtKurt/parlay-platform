...
export function createServer({ config = loadConfig(), authorizer, store, queue } = {}) {
  store ||= new JobStore(config.dataDir); authorizer ||= createAuthorizer(config); queue ||= new DurableQueue(store, createRunner(config, store));
  for (const name of fs.readdirSync(config.dataDir).filter((x) => x.endsWith('.json'))) {
    const recovered = store.get(name.slice(0, -5));
    if (recovered && ['queued', 'running'].includes(recovered.status)) { recovered.status = 'queued'; store.save(recovered); queue.enqueue(recovered.id); }
  }
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      let body = {}; if (['POST','PUT'].includes(request.method)) { let raw=''; for await (const chunk of request) { raw += chunk; if (raw.length > config.maxInstructionBytes + 10000) throw Object.assign(new Error('request_too_large'), { status: 413 }); } body = raw ? JSON.parse(raw) : {}; }
      ...
      if (request.method === 'POST' && !id) {
        ...
        if (!Array.isArray(body.authorizedScope) || !body.authorizedScope.length || body.authorizedScope.some((x) => typeof x !== 'string' || x.includes('..') || x.startsWith('/'))) return send(400, { error: 'valid_authorized_scope_required' });
      ...
      if (request.method === 'POST' && action === 'continue') { if (!['completed','failed','blocked','awaiting_approval'].includes(job.status)) return send(409, { error: 'job_not_continuable' }); if (!String(body.instruction || '').trim()) return send(400, { error: 'instruction_required' }); job.instruction = String(body.instruction); job.status = 'queued'; job.cancelRequested = false; store.save(job); queue.enqueue(id); return send(202, { job: publicJob(job) }); }
