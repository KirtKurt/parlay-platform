...
export function createAuthorizer(config, verify = null) {
  const jwks = createRemoteJWKSet(new URL(`${config.issuer.replace(/\/$/, '')}/.well-known/jwks.json`));
  const verifyToken = verify || ((token) => jwtVerify(token, jwks, { issuer: config.issuer, audience: config.audience }));
  return async function authorize(request) {
    ...
    let payload;
    try { ({ payload } = await verifyToken(value.slice(7))); }
    catch { throw Object.assign(new Error('invalid_access_token'), { status: 401 }); }
