...
async function refresh() {
  try {
    ...
    for (const job of result.jobs) {
      ...
      const failedPublication = job.status === 'failed' && job.publicationState && job.publicationState !== 'no_changes';
      if (['queued', 'running', 'awaiting_publication', 'published'].includes(job.status) || failedPublication) { const cancel = document.createElement('button'); cancel.textContent = job.cancelRequested ? 'Cancellation pending' : 'Cancel'; cancel.disabled = Boolean(job.cancelRequested); cancel.onclick = async () => { try { await api(`/${job.id}/cancel`, {}); await refresh(); } catch (e) { message.textContent = e.message; } }; item.append(cancel); }
