...
class _JsonHttpClient:
    def __init__(self, *, provider: str, base_url: str, timeout_seconds: int, max_attempts: int = DEFAULT_HTTP_ATTEMPTS, opener: Callable[..., Any] = urllib.request.urlopen, sleeper: Callable[[float], None] = time.sleep) -> None:
    ...
    def request_json(self, *, endpoint: str, params: Mapping[str, Any] | None = None, headers: Mapping[str, str] | None = None) -> tuple[Any, TransportMeta]:
        ...
        request = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": "inqis-nfl-auto/1.0", **dict(headers or {})}, method="GET")
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                ...
                meta = TransportMeta(provider=self.provider, endpoint=endpoint, status=status, rate_limit=response_headers.get("x-ratelimit-limit"), rate_remaining=response_headers.get("x-ratelimit-remaining"), rate_reset=response_headers.get("x-ratelimit-reset"), requests_remaining=response_headers.get("x-requests-remaining"), requests_used=response_headers.get("x-requests-used"), requests_last=response_headers.get("x-requests-last"))
                return payload, meta
...
class BBDClient:
    ...
    def __init__(self, *, api_key: str | None = None, secret_arn: str | None = None, secrets_client: Any = None, base_url: str = "https://api.bigballsdata.com", timeout_seconds: int = DEFAULT_BBD_TIMEOUT_SECONDS, max_attempts: int = DEFAULT_HTTP_ATTEMPTS, opener: Callable[..., Any] = urllib.request.urlopen, sleeper: Callable[[float], None] = time.sleep) -> None:
        self.api_key = api_key or resolve_secret(str(secret_arn or ""), client=secrets_client)
        self.http = _JsonHttpClient(provider="BBD", base_url=base_url, timeout_seconds=timeout_seconds, max_attempts=max_attempts, opener=opener, sleeper=sleeper)

    def _get(self, endpoint: str, params: Mapping[str, Any] | None = None) -> tuple[dict[str, Any], TransportMeta]:
        payload, meta = self.http.request_json(endpoint=endpoint, params=params, headers={"Authorization": f"Bearer {self.api_key}"})
    ...
    def list_games_page(self, *, season: int, game_type: str, week: int | None = None, team: str | None = None, limit: int = 200, offset: int = 0) -> tuple[list[dict[str, Any]], dict[str, Any], TransportMeta]:
        ...
        payload, meta = self._get("/v1/nfl/games", {"season": int(season), "type": normalized_type, "week": week, "team": team, "limit": min(max(int(limit), 1), 200), "offset": max(int(offset), 0)})
    ...
    def iter_games(self, *, season: int, game_type: str) -> Iterator[tuple[dict[str, Any], TransportMeta]]:
        offset = 0
        while True:
            rows, pagination, meta = self.list_games_page(season=season, game_type=game_type, limit=200, offset=offset)
            for row in rows:
                yield row, meta
            offset += len(rows)
    ...
    def list_plays_page(self, *, game_id: str, limit: int = 500, offset: int = 0) -> tuple[list[dict[str, Any]], dict[str, Any], TransportMeta]:
        safe_id = urllib.parse.quote(str(game_id).strip(), safe="_-." )
        if not safe_id:
            raise ContractError("BBD_NFL_GAME_ID_MISSING")
        payload, meta = self._get(f"/v1/nfl/games/{safe_id}/plays", {"limit": min(max(int(limit), 1), 500), "offset": max(int(offset), 0)})
        rows = payload.get("data")
        if not isinstance(rows, list) or any(not isinstance(row, Mapping) for row in rows):
    ...
    def all_plays(self, *, game_id: str, max_pages: int = 12) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        offset = 0; pages = 0; rows: list[dict[str, Any]] = []; transport: list[dict[str, Any]] = []
        while pages < max_pages:
            page, pagination, meta = self.list_plays_page(game_id=game_id, limit=500, offset=offset)
            rows.extend(page); transport.append(meta.to_dict()); offset += len(page); pages += 1
            total = int(pagination.get("total") or offset)
...
class OddsApiClient:
    ...
    def __init__(self, *, api_key: str | None = None, secret_arn: str | None = None, secrets_client: Any = None, base_url: str = "https://api.the-odds-api.com", timeout_seconds: int = DEFAULT_ODDS_TIMEOUT_SECONDS, max_attempts: int = DEFAULT_HTTP_ATTEMPTS, opener: Callable[..., Any] = urllib.request.urlopen, sleeper: Callable[[float], None] = time.sleep) -> None:
        self.api_key = api_key or resolve_secret(str(secret_arn or ""), client=secrets_client)
        self.http = _JsonHttpClient(provider="ODDS_API", base_url=base_url, timeout_seconds=timeout_seconds, max_attempts=max_attempts, opener=opener, sleeper=sleeper)
    ...
    def historical_odds(self, *, snapshot_at: str, markets: tuple[str, ...] = ODDS_MARKETS, regions: str = ODDS_REGIONS) -> tuple[dict[str, Any], TransportMeta]:
        payload, meta = self._get(f"/v4/historical/sports/{SPORT_KEY}/odds", {"date": snapshot_at, "regions": regions, "markets": ",".join(markets), "oddsFormat": "decimal", "dateFormat": "iso"})
    ...
    def historical_events(self, *, snapshot_at: str) -> tuple[dict[str, Any], TransportMeta]:
        payload, meta = self._get(f"/v4/historical/sports/{SPORT_KEY}/events", {"date": snapshot_at, "dateFormat": "iso"})
        if not isinstance(payload, Mapping) or not isinstance(payload.get("data"), list):
            raise ContractError("ODDS_API_HISTORICAL_EVENTS_INVALID")
        return dict(payload), meta

    def live_odds(self, *, markets: tuple[str, ...] = ODDS_MARKETS, regions: str = ODDS_REGIONS) -> tuple[list[dict[str, Any]], TransportMeta]:
        payload, meta = self._get(f"/v4/sports/{SPORT_KEY}/odds", {"regions": regions, "markets": ",".join(markets), "oddsFormat": "decimal", "dateFormat": "iso"})
    ...
    def scores(self, *, days_from: int = 3) -> tuple[list[dict[str, Any]], TransportMeta]:
        payload, meta = self._get(f"/v4/sports/{SPORT_KEY}/scores", {"daysFrom": min(max(int(days_from), 1), 3), "dateFormat": "iso"})
        if not isinstance(payload, list) or any(not isinstance(row, Mapping) for row in payload):
