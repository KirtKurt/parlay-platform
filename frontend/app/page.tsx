...
function WorkingCard({ title, copy }: { title: string; copy: string }) {
  return <div className="inqsi-empty"><strong>{title}</strong><span>{copy}</span></div>;
}
  ...
  return (
    <main className="inqsi-shell">
      <header className="inqsi-topbar">
        <Link className="inqsi-brand" href="/"><span className="inqsi-logo-mark">Q</span><span><b>InQsi</b><small>Market Intelligence</small></span></Link>
        <nav className="inqsi-nav-actions"><a href="#performance">Performance</a><a href="#signup">Log in</a><a className="inqsi-primary" href="#signup">Start 5 days free</a></nav>
      </header>
      ...
      <section className="inqsi-hero">
        <div className="inqsi-hero-card">
          ...
          <div className="inqsi-stat-grid"><div><b>15 min</b><span>market snapshots</span></div><div><b>3 min</b><span>close-to-live mode</span></div><div><b>8 combos</b><span>3-leg outcome ranking</span></div><div><b>1 hour</b><span>winner lean window</span></div></div>
        </div>
        <aside className="inqsi-signup-card">
          <h2>Start with 5 days free</h2><p>Save watchlists, alerts, slip scans, dashboards, and InQsi winner leans.</p><a href="#signup">Continue with Google</a><a href="#signup">Continue with Apple</a><a className="inqsi-primary" href="#signup">Create account</a><small>Google and Apple sign-in are ready visually. Provider keys still need to be connected.</small>
        </aside>
      </section>

      <section className="inqsi-tabs">{SPORTS.map((sport) => <a key={sport} href={`#${sport}`}>{sport}</a>)}</section>
      ...
      <section className="inqsi-seo-links">{['Predicted winners','Best lines','Parlay scanner','Live market mode','Public performance','Alerts','Closing line value','Watchlist','News context','X / Twitter','Instagram','TikTok','YouTube','Discord','LinkedIn'].map((label) => <a key={label} href="#signup">{label}</a>)}</section>
      <section id="signup" className="inqsi-auth-modal"><div><a className="inqsi-close" href="#">×</a><h2>Start 5 days free</h2><p>Sign up or log in to save your dashboard.</p><button>Continue with Google</button><button>Continue with Apple</button><input placeholder="Email address" type="email" /><button className="inqsi-primary">Continue</button><small>Working on it: OAuth provider keys still need to be connected before live sign-in.</small></div></section>
    </main>
  );
}
