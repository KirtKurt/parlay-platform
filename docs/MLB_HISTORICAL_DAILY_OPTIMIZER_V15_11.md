...
V15.11.1 extends the V15.1 multi-league platform with a fail-closed MLB historical
...
Before cutover, the public MLB API deliberately preserves the existing V15.10
model/runtime identity strings used by the production cold-start and deployment
attestation checks. A separately versioned historical extension reports the
optimizer and authority lifecycle. This lets the fail-closed loader be deployed
without changing the incumbent selector or breaking current production identity
contracts. After cutover, the endpoint reports the historical champion as the
sole model and authority while retaining the same API compatibility surface.
...
## One-time production deployment authorization

The GitHub workflow does not treat every future `main` push as permission to
spend paid historical-data credits. A main-branch cutover deployment runs only
when the merge commit carries `[deploy-mlb-v15.11]`; paid The Odds API backfill
also requires `[authorize-mlb-historical-backfill]` in that same commit. This
creates a reviewable one-time authorization trail while preserving the existing
manual `workflow_dispatch` confirmations for later operator-initiated runs.

The authorization permits deployment and collection only. It does **not**
permit promotion by assertion. V15.10 remains the incumbent until the persisted
V15.11 artifact independently passes the 1,000/200/200 chronological,
complete-slate, every-held-out-day-at-least-80% and anti-overfit gates. The
write-once historical-only cutover is created only at that point.

## V15.11.1 prediction-only safety boundary
