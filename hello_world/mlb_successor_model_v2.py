...
Named starter rates, batting-order season OPS, prior-day relief workload, and
two recorded movement features adjust it. V1 protocol and records remain intact.
...
import mlb_statsapi_team_context as team_source

VERSION = "MLB-SUCCESSOR-MARKET-OFFSET-TEAM-CONTEXT-v2"
EXPERIMENT_ID = "mlb-successor-2026-09-09-team-context-v2"
FEATURES = ("deltaGapHome", "homeAwayVelocityPpHr60mDiff", "starterEraGapHome",
            "starterKMinusBbGapHome", "starterRatesMissing",
            "lineupOpsGapHome", "bullpenPitches3dGapHome", "lineupOpsMissing", "bullpenWorkloadMissing")
PROTOCOL = {
    "version": VERSION, "experimentId": EXPERIMENT_ID,
    "features": list(FEATURES), "penalties": [0.1, 1.0, 10.0],
    "teamObservedTrainMinimum": 100, "teamObservedCalibrationMinimum": 25,
    "teamObservedSelectionMinimum": 25,
    "predecessorExperimentId": "mlb-successor-2026-09-09-starter-rates-v1",
    "teamFeaturesRequireObservedSources": True,
...
def record(row, *, labeled):
    ...
    lineup = (snap.get("groups") or {}).get("confirmed_lineups") or {}
    bullpen = (snap.get("groups") or {}).get("bullpen_availability") or {}
    lv, bv = lineup.get("values") or {}, bullpen.get("values") or {}
    ops = [number(lv.get(side + "MeanSeasonOps")) for side in ("home", "away")]
    def workload(side):
        usage = bv.get(side + "Usage1d3d5d")
        window = usage.get("3d") if isinstance(usage, dict) else None
        return number(window.get("pitches")) if isinstance(window, dict) else None
    pitches = [workload(side) for side in ("home", "away")]
    def trusted(group):
        return group.get("status") in snapshots.SOURCE_PRESENT_STATUSES and group.get("dataset") == team_source.VERSION
    lineup_observed = (trusted(lineup) and all(lv.get(side + "Confirmed") is True for side in ("home", "away"))
                       and all(v is not None and 0 <= v <= 5 for v in ops))
    bullpen_observed = trusted(bullpen) and all(v is not None and v >= 0 and v.is_integer() for v in pitches)
    result.update(lineupOpsGapHome=ops[0]-ops[1] if lineup_observed else None,
                  bullpenPitches3dGapHome=pitches[0]-pitches[1] if bullpen_observed else None,
                  lineupOpsMissing=0.0 if lineup_observed else 1.0,
                  bullpenWorkloadMissing=0.0 if bullpen_observed else 1.0)
...
def development(rows):
    ...
    team_observed = {k: sum(r.get("lineupOpsMissing") == 0 and r.get("bullpenWorkloadMissing") == 0 for r in v) for k, v in groups.items()}
    blockers = []
    ...
    for group, requirement in (("train", "teamObservedTrainMinimum"), ("calibration", "teamObservedCalibrationMinimum"), ("selection", "teamObservedSelectionMinimum")):
        if team_observed[group] < PROTOCOL[requirement]:
            blockers.append("INSUFFICIENT_OBSERVED_TEAM_CONTEXT_" + group.upper())
    report = {"ok": True, "status": "ACCUMULATING_TEAM_CONTEXT_DEVELOPMENT_DATA",
    ...
    for penalty in PROTOCOL["penalties"]:
        ...
        for calibrated in (False, True):
            candidate = {"name": f"team_context_offset_l2_{penalty}_cal_{calibrated}", "model": model, "calibrator": None}
            if calibrated:
