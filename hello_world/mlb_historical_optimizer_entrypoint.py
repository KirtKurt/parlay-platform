...
def _cross_date_evidence(raw: Mapping[str, Any], slate_date: str) -> Dict[str, Any]:
    ...
    return {"officialGamePk": str(raw.get("gamePk") or ""), "queriedSlateDateEt": slate_date,
            "officialDate": str(raw.get("officialDate") or ""), "gameDate": raw.get("gameDate"),
            "rescheduleDate": raw.get("rescheduleDate"), "resumeDate": raw.get("resumeDate"),
            "rescheduledFrom": raw.get("rescheduledFrom"), "resumeGameDate": raw.get("resumeGameDate"),
            "awayTeam": _team_name(raw, "away"), "homeTeam": _team_name(raw, "home"),
            "officialStatus": {"abstractGameState": status.get("abstractGameState"),
...
def fetch_official_schedule_cross_date_safe(slate_date: str, *, timeout: int = 15,
        http_get: Optional[Callable[[str, int], Any]] = None) -> Dict[str, Any]:
    ...
    if not isinstance(payload, dict): raise RuntimeError("MLB_OFFICIAL_FINAL_PAYLOAD_NOT_OBJECT")
    dates = payload.get("dates")
    if not isinstance(dates, list): raise RuntimeError("MLB_OFFICIAL_FINAL_DATES_INVALID")
    ...
    for date_row in dates:
        ...
        if not isinstance(games, list): raise RuntimeError("MLB_OFFICIAL_FINAL_GAMES_INVALID")
        kept = []
        for raw in games:
            if not isinstance(raw, dict): raise RuntimeError("MLB_OFFICIAL_FINAL_GAME_ROW_INVALID")
            provider_game_count += 1
            game_pk = str(raw.get("gamePk") or "").strip()
            if not game_pk or game_pk in seen: raise RuntimeError("MLB_OFFICIAL_FINAL_GAME_PK_INVALID_OR_DUPLICATE")
            seen.add(game_pk)
            ...
            if official_date != slate_date:
                ...
                if not evidence["officialDate"]: raise RuntimeError(f"MLB_OFFICIAL_FINAL_CROSS_DATE_IDENTITY_UNPROVEN:{game_pk}")
                exclusions.append(evidence)
            else:
                kept.append(copy.deepcopy(raw))
        row = copy.deepcopy(date_row); row["games"] = kept; row["totalGames"] = len(kept); filtered_dates.append(row)
    filtered = copy.deepcopy(payload); filtered["dates"] = filtered_dates
    ...
    canonical.update({"crossDateCanonicalizationVersion": VERSION,
                      "providerReportedGameCount": provider_game_count,
                      "crossDateExcludedCount": len(exclusions), "crossDateExclusions": exclusions,
