...
def enforce_structure(ranked: List[Dict[str, Any]], regime: str) -> List[Dict[str, Any]]:
    by_dogs = {0: [], 1: [], 2: [], 3: []}
    for r in ranked:
        by_dogs[int(r["underdogs"])].append(r)

    for k in by_dogs:
        by_dogs[k].sort(key=lambda x: (x["score"], x["combo_prob"]), reverse=True)
    ...
    rest.sort(key=lambda x: (x["score"], x["combo_prob"]), reverse=True)
...
def rank_nba_b11c1(games_input: List[Dict[str, Any]]) -> Dict[str, Any]:
    if len(games_input) != 3:
        raise ValueError("NBA-B1.1C.1 expects exactly 3 games")

    games = [normalize_game(g) for g in games_input]
    regime = detect_regime(games)

    combos = [list(c) for c in product(*[[g.home, g.away] for g in games])]
    ...
    rows.sort(key=lambda x: (x["score"], x["combo_prob"]), reverse=True)
    rows = enforce_structure(rows, regime)
