...
def latest_pull_for(sport: str, slate_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {"ok": True, "sport": sport, "pullCount": len(pulls), "latestPull": pull}
...
def board_for_sport(sport: str, slate_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    games = [game_row(g) for g in (pull.get("games") or [])]
    return {
        "ok": True,
        "sport": sport,
        ...
        "games": games,
        "note": "Market board reads only the latest stored official provider pull. Member-uploaded slips are not included.",
    }
...
def board_all(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "ok": True,
        "board": "market_board_latest_pull",
...
def render_page(board: Dict[str, Any]) -> str:
    ...
    for game in board.get("games", []) or []:
        ...
        """)
    body = "".join(cards) or "<div class='empty'>No pulled games yet for this sport.</div>"
    return f"""
    ...
    <style>
      ...
      <div class='sub'>Latest pull: {html.escape(str(board.get('latestPulledAt') or 'none'))} · Games: {html.escape(str(board.get('gameCount') or 0))}</div>
      {body}
    </div></body></html>
    """
