...
def _standardize(records: Sequence[Dict[str, Any]], features: Sequence[str]) -> Tuple[Dict[str, float], Dict[str, float]]:
    means: Dict[str, float] = {}; scales: Dict[str, float] = {}
...
def fit_logistic(records: Sequence[Dict[str, Any]], features: Sequence[str], target: str, version: str,
    ...
    means, scales = _standardize(rows, features); weights = {feature: 0.0 for feature in features}
...
def train(clean_rows: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    records = records_from_clean_rows(clean_rows); split = walk_forward.split_chronological(records)
    if not split.get("ok"):
        return {"ok": False, "version": VERSION, "recordCount": len(records),
    ...
    reliability_model["selectedThreshold"] = {**threshold, "threshold": selected_threshold}; reliability_model["thresholdSelectedOnValidationOnly"] = True
    outcome_model["directionThreshold"] = 0.5
    return {"ok": True, "version": VERSION, "recordCount": len(records),
            "split": {key: value for key, value in split.items() if key not in {"train", "validation", "test"}},
