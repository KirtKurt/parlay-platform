...
class EngineeringController:
    ...
    def choose_actions(self, snapshot: Snapshot) -> List[Action]:
        ...
        if not snapshot.production_ok:
            actions.append(Action("dispatch", "production_health_failed:" + ",".join(snapshot.production_failures), KNOWN_WORKFLOWS["repair"]))
        elif snapshot.deploy_conclusion == "failure":
            actions.append(Action("dispatch", "latest_main_deploy_failed", KNOWN_WORKFLOWS["repair"]))
        elif snapshot.latency_conclusion == "failure":
            actions.append(Action("issue", "latency_acceptance_gate_failed", "latency"))
        elif snapshot.latency_p95_ms is not None and snapshot.latency_p95_ms > 500:
            actions.append(Action("issue", f"latency_p95_above_gate:{snapshot.latency_p95_ms:.1f}ms", "latency"))
        elif not snapshot.code_agent_configured:
            actions.append(Action("issue", "code_agent_not_configured; bounded operations remain active", "code-agent"))
        else:
            actions.append(Action("code-agent", "advance_highest_priority_ARB_BACKLOG_item", "inqsi-arb/ARB_BACKLOG.md"))
        ...
        for action in actions:
            ...
            if action.kind == "issue":
                ...
                body = (
                    ...
                    "This issue was created by the bounded Inqsi ARB autonomous engineering controller. "
                    "It must not weaken fail-closed settlement, freshness, identity, security, or no-wager-placement protections."
                )
                issue = self.gh.create_issue(title, body)
                results.append({"action": asdict(action), "status": "created", "issue": issue.get("number")})
                continue
            if action.kind == "code-agent":
...
def main() -> int:
    ...
    hard_failure = any(row.get("status") == "failed" for row in results)
