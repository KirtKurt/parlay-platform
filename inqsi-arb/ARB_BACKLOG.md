...
1. **Provider/context integration**
   ...
   - 2026-09-15 HTTP-error handling increment (item 1 only): BBD transport now closes HTTP error responses without reading their untrusted bodies and emits status-only exceptions, suppressing chained upstream diagnostics. Offline urllib regressions cover 401/403/404/429/500/503 across health, sports and events, preserving access-failure reasons and empty context, and verify direct-request exception redaction and response closure. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (534 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent transport safeguard does not establish live authentication or entitlement or enable runtime wiring. The Odds API remains the sportsbook-price authority.
