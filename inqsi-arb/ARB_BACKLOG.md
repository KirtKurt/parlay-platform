...
1. **Provider/context integration**
   ...
   - 2026-09-26 bounded-response increment (item 1 only): BBD successful response bodies are limited to 1 MiB, reading at most one extra byte to detect overflow. Oversized bodies return `BBD_RESPONSE_TOO_LARGE` before JSON decoding and expose no context, regardless of missing or inaccurate Content-Length. HTTP error diagnostics read at most 500 bytes, optional access failures skip the body, and error responses close while preserving existing access-failure reasons. Offline regressions cover health, sports and events, exact-limit acceptance, oversized-body rejection and HTTP error handling. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (802 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (build succeeded; global SAM metadata writes blocked by the read-only home filesystem). The conservative size limit is an adapter safety bound, not a proven provider page-size contract. This increment does not qualify live provider behavior or enable runtime wiring; The Odds API remains the sportsbook-price authority.
