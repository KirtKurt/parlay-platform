...
1. **Provider/context integration**
   ...
   - 2026-09-15 numeric-overflow increment (item 1 only): BBD JSON decoding now rejects numeric literals that overflow to non-finite floats (for example `1e400`) with `BBD_RESPONSE_JSON_INVALID` and no context. Offline health, sports and events regressions cover positive/negative overflow and nested fields, verify response closure, and preserve finite numeric values and quoted exponent strings. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (523 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not establish live provider qualification or enable runtime wiring. The Odds API remains the sportsbook-price authority.
