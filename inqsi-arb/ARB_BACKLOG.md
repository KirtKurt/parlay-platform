...
1. **Provider/context integration**
   ...
   - 2026-09-15 event-identity increment (item 1 only): event collections now fail closed with `BBD_EVENT_ID_INVALID` for missing, malformed or conflicting identity aliases and `BBD_EVENT_ID_DUPLICATE` for repeated normalized IDs, returning no partial context. Each supplied `id`/`match_id`/`event_id` must be an integer (excluding booleans) or nonblank string without surrounding whitespace; supplied aliases must agree after integer-to-string conversion. Zero and opaque string IDs, including leading zeros, are preserved. Offline regressions cover invalid rows after valid context, duplicate IDs with conflicting statuses, alias agreement and distinct opaque IDs. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (539 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). These conservative local identity checks do not prove live schema or cross-provider identity and do not enable runtime wiring. The Odds API remains the sportsbook-price authority.
