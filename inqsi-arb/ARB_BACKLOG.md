...
1. **Provider/context integration**
   ...
   - 2026-09-17 event-identity increment (item 1 only): event context now requires a nonempty, whitespace/control-free string or nonnegative integer ID from `id`, `match_id` or `event_id`. Every present alias must be valid and agree after integer-to-string conversion; malformed, missing or conflicting IDs return `BBD_EVENT_ID_INVALID` and discard the entire event collection. Numeric zero and opaque string formatting are preserved. Offline regressions cover invalid types, conflicting aliases, partial-collection rejection and accepted identities. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (602 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This conservative local identity contract does not establish live provider IDs, cross-provider identity matching or entitlement, and does not enable runtime wiring. The Odds API remains the sportsbook-price authority.
