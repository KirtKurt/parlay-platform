...
1. **Provider/context integration**
   ...
   - 2026-09-18 bounded-response increment (item 1 only): BBD successful response reads are capped at 2 MiB plus one detection byte before JSON decoding. Oversized responses return `BBD_RESPONSE_TOO_LARGE` with no context; complete responses at the limit remain accepted. HTTP error diagnostics read at most 500 bytes, optional access failures require no body read, and error responses are closed while preserving access-failure reasons. Offline regressions cover health, sports and events, exact-limit acceptance, oversized rejection, bounded read requests and HTTP-error closure/status handling. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (599 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). The cap is a local safety policy, not a proven provider page-size limit; larger legitimate responses fail closed pending provider pagination qualification. The Odds API remains the sportsbook-price authority; runtime wiring stays pending.
