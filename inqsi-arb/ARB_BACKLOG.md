...
1. **Provider/context integration**
   ...
   - 2026-09-13 sports HTTP-status increment (item 1 only): sports discovery now returns no context for non-200 responses, including HTTP 206 partial content, with `BBD_SPORTS_ENDPOINT_UNAVAILABLE_OR_UNENTITLED` for decoded responses. Status rejection precedes collection-schema validation; existing transport failures retain their reasons. Offline urllib regressions cover eight non-200 success statuses with populated, empty, unrecognized and absent envelopes, response closure, and preservation of valid HTTP 200 collections. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (528 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). The Odds API remains the sportsbook-price authority; runtime wiring remains pending.
