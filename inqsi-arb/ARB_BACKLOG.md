...
1. **Provider/context integration**
   ...
   - 2026-09-12 collection-envelope increment (item 1 only): reject multiple recognized collection keys at either supported envelope level with `BBD_COLLECTION_SCHEMA_INVALID`, including empty, null and malformed competing candidates, instead of silently selecting the first collection. Offline health, sports and events regressions cover conflicting data, key order, no returned context, and preservation of all previously supported unambiguous envelopes with metadata. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (618 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not qualify live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
