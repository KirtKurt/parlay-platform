...
1. **Provider/context integration**
   ...
   - 2026-09-12 HTTP-error transport increment (item 1 only): BBD HTTP error responses are now always closed without reading their untrusted bodies, and adapter errors retain only the HTTP status. Offline regressions cover 400/401/403/404/429/500/503 across health, sports and events, preserve optional access-failure diagnostics, and prove error bodies are never read. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (438 passed); `SAM_CLI_TELEMETRY=0 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (build succeeded, exit 0; global SAM metadata writes blocked by the read-only home filesystem). No runtime context wiring or live provider qualification is enabled by this increment.
