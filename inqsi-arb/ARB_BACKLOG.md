...
1. **Provider/context integration**
   ...
   - 2026-09-17 response-size increment (item 1 only): successful BBD response bodies are limited to 2 MiB, with a bounded limit-plus-one-byte read. Oversized bodies return `BBD_RESPONSE_TOO_LARGE` before JSON decoding and expose no context. Offline health, sports and events tests cover exact-limit acceptance, oversized rejection, bounded reads and response closure. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (586 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). The cap is a local safety policy, not a verified provider page-size contract; live qualification must establish compatible pagination before runtime wiring. The Odds API remains the sportsbook-price authority.
