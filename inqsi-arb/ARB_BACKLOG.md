...
1. **Provider/context integration**
   ...
   - 2026-09-12 collection-ambiguity increment (item 1 only): BBD collection parsing now requires exactly one recognized collection field at each supported envelope level. Conflicting collections, including empty or malformed siblings, fail closed with `BBD_COLLECTION_SCHEMA_INVALID` instead of silently selecting the first field. Offline regressions cover health, sports and events; all supported single-collection envelopes retain rows and allow unrelated metadata. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (552 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not establish live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
