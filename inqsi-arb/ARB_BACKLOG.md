...
1. **Provider/context integration**
   ...
   - 2026-09-13 collection-alias increment (item 1 only): BBD collection parsing now requires exactly one recognized collection key at each supported envelope level. Conflicting aliases, including empty or malformed alternatives, return `BBD_COLLECTION_SCHEMA_INVALID` and no context instead of silently selecting the first collection. Offline regressions cover health, sports and events, plus every previously supported unambiguous outer/nested alias with metadata. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (555 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not qualify live BBD context or enable runtime wiring. The Odds API remains the sportsbook-price authority.
