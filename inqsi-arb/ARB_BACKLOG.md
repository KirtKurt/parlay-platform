...
1. **Provider/context integration**
   ...
   - 2026-09-12 competing-envelope increment (item 1 only): BBD collection parsing now rejects multiple recognized collection keys at either supported envelope level with `BBD_COLLECTION_SCHEMA_INVALID`, including empty, malformed and conflicting alternatives, rather than silently selecting the first key. Single recognized envelopes, empty collections and unrelated metadata remain supported. Offline regressions cover health, sports and events, key-order independence and valid envelope variants. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (542 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not qualify live context or enable runtime wiring. The Odds API remains the sportsbook-price authority.
