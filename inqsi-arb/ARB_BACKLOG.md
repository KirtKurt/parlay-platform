...
1. **Provider/context integration**
   ...
   - 2026-09-19 interrupted-response increment (item 1 only): BBD body reads now convert incomplete HTTP reads, disconnects, connection resets and timeouts into `BBD_REQUEST_FAILED` with no returned context. Both successful and HTTP-error responses close on read failure; complete HTTP-error responses also close while preserving existing access-failure reasons. Offline regressions cover health, sports and events across HTTP 200/401/403/404/429/500. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (654 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent transport safeguard does not qualify live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
