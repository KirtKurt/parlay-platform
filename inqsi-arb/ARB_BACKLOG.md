...
1. **Provider/context integration**
   ...
   - 2026-09-18 HTTP-status increment (item 1 only): BBD sports discovery now discards context from non-200 responses before collection parsing, returning an explicit unavailable/unentitled reason and the HTTP status. Offline urllib regressions cover 201/202/204/205/206/299 across health, sports and events with both populated and invalid collection envelopes, verify response closure, and retain a populated HTTP-200 sports success case. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (614 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent status safeguard does not establish live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
