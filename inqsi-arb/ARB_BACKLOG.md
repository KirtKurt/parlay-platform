...
1. **Provider/context integration**
   ...
   - 2026-09-15 collection-ambiguity increment (item 1 only): BBD collection parsing now requires exactly one recognized collection field at each supported envelope level. Conflicting aliases, including empty, null or malformed alternatives, return `BBD_COLLECTION_SCHEMA_INVALID` without context instead of selecting by field precedence. Offline regressions cover health, sports and events and preserve single-envelope empty collections with metadata. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (564 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not qualify live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
