...
1. **Provider/context integration**
   ...
   - 2026-09-16 numeric-overflow increment (item 1 only): BBD JSON decoding now rejects numeric literals that overflow to non-finite floats (for example `1e999`) with `BBD_RESPONSE_JSON_INVALID`, including nested fields and metadata. Offline health, sports and events regressions verify no returned context and response closure; finite boundary values, ordinary numbers and quoted exponents remain accepted. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (523 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not qualify live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
   - 2026-09-17 collection-ambiguity increment (item 1 only): BBD collection parsing now requires exactly one recognized collection field at each supported envelope level. Competing fields (including null, malformed or identical empty alternatives) return `BBD_COLLECTION_SCHEMA_INVALID` with no context instead of silently selecting the first collection. Offline health, sports and events regressions cover every recognized field pair in both insertion orders; single-field envelopes still preserve rows and metadata. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (577 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not qualify live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
   - 2026-09-26 event-identity increment (item 1 only): event context now requires at least one `id`, `match_id` or `event_id`; every present alias must be an integer (excluding booleans) or a nonempty string without whitespace/control characters, and all aliases must agree after integer-to-string normalization. Missing, malformed or conflicting IDs return `BBD_EVENT_ID_INVALID` and discard the entire response's event context, including otherwise valid rows. Opaque string IDs remain unchanged; integer zero is preserved instead of being lost through truthiness fallback. Offline regressions cover invalid types, conflicting aliases, matching aliases and supported scalar IDs. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (810 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This local identity contract does not establish provider ID semantics or qualify live context; runtime wiring remains pending and The Odds API remains the sportsbook-price authority.
