...
1. **Provider/context integration**
   ...
   - 2026-09-12 numeric-overflow increment (item 1 only): BBD JSON decoding now rejects exponent numbers that overflow to positive or negative infinity with `BBD_RESPONSE_JSON_INVALID`, including nested fields, and returns no context. Offline health, sports and events regressions verify failure reasons and response closure; finite boundary values, ordinary numbers and quoted exponents remain accepted. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (505 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent parsing safeguard does not qualify live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
