...
1. **Provider/context integration**
   ...
   - 2026-09-14 bounded-response increment (item 1 only): BBD successful response bodies are limited to 2 MiB before UTF-8/JSON decoding; oversized bodies fail closed with `BBD_RESPONSE_TOO_LARGE` and no context. HTTP error details read at most 500 bytes, optional access failures skip body reads, and HTTP error responses are explicitly closed while preserving access-failure reasons. Offline urllib tests cover the exact size boundary, oversized bodies and HTTP 401/403/404/429/500 across health, sports and events. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (534 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). The size cap is a local safety limit, not evidence of live provider page sizes or pagination completeness. The Odds API remains the sportsbook-price authority; runtime wiring remains pending.
