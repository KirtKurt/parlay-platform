...
1. **Provider/context integration**
   ...
   - 2026-09-19 sports HTTP-status increment (item 1 only): BBD sports discovery now discards all context from non-200 HTTP responses and reports `BBD_SPORTS_ENDPOINT_UNAVAILABLE_OR_UNENTITLED`, preserving the response status. Previously, valid collection rows from responses such as HTTP 206 were returned alongside `ok: false`. Offline transport regressions cover HTTP 201/202/204/206 with populated, unrecognized and empty bodies, response closure, and preservation of valid HTTP 200 context. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (590 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent status safeguard does not qualify live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
