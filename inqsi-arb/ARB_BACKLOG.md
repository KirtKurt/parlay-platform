...
1. **Provider/context integration**
   ...
   - 2026-09-16 bounded-response increment (item 1 only): BBD success-body reads are capped at 1 MiB plus one detection byte, independent of Content-Length. Oversized responses fail closed with `BBD_RESPONSE_TOO_LARGE` before JSON parsing and return no context; exactly-at-limit valid collections remain accepted. HTTP error responses are closed without reading their bodies, preserving status/access failure reasons and excluding provider body details. Offline transport regressions cover health, sports and events, absent/inaccurate Content-Length, the size boundary, and 401/403/404/429/500 response cleanup. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (537 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (build succeeded; global SAM metadata writes blocked by the read-only home filesystem). The 1 MiB limit is a local safety policy, not a proven provider page-size contract; larger responses remain unavailable pending qualified pagination. No live authentication/entitlement proof or runtime wiring is claimed; The Odds API remains the sportsbook-price authority.
