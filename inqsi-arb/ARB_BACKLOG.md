...
1. **Provider/context integration**
   ...
   - 2026-09-19 bounded-response increment (item 1 only): successful BBD HTTP response bodies are limited to 1 MiB, buffering at most one additional byte to detect overflow before UTF-8/JSON decoding. Oversized bodies return `BBD_RESPONSE_TOO_LARGE` with no context. Offline health, sports and events regressions cover below/exact/above-limit bodies, bounded reads, response closure and rejection before JSON decoding. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (592 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). The cap is a local safety policy, not a verified provider page-size contract; live response sizes and pagination still need qualification. This increment does not enable runtime wiring or change The Odds API price authority.
