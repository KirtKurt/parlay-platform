...
1. **Provider/context integration**
   ...
   - 2026-09-25 bounded-response increment (item 1 only): BBD transport reads at most 1 MiB plus one detection byte and rejects oversized successful responses before JSON decoding with `BBD_RESPONSE_TOO_LARGE`; no partial context is returned. HTTP error bodies are neither read nor included in adapter diagnostics, and error responses are explicitly closed while preserving access-failure status/reason precedence. Offline health, sports and events regressions cover just below/at/above the byte limit, bounded reads, response closure, and 401/403/404/429/500 errors. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (799 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). The limit is an offline safety bound, not a verified live provider payload limit; larger collections will require qualified pagination. Runtime wiring remains pending and The Odds API remains the sportsbook-price authority.
