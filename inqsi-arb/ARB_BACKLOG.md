...
1. **Provider/context integration**
   ...
   - 2026-09-18 event-identity increment (item 1 only): BBD event context now requires a nonempty string or integer identity from `id`, `match_id` or `event_id`. Every present alias must be valid and agree after integer-to-string normalization; missing IDs, null/blank aliases, booleans, floats, containers and conflicting aliases return `BBD_EVENT_ID_INVALID` with no partial event collection. Integer zero and string formatting are preserved. Offline regressions exercise invalid rows before and after valid rows, alternate aliases and agreeing aliases. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (610 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent identity safeguard does not prove live IDs or qualify context for runtime wiring. The Odds API remains the sportsbook-price authority.
