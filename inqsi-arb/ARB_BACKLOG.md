...
1. **Provider/context integration**
   ...
   - 2026-09-16 numeric-overflow increment (item 1 only): BBD JSON decoding now rejects numeric literals that overflow to infinity (for example `1e309`) with `BBD_RESPONSE_JSON_INVALID`, including nested fields and metadata. Offline regressions cover health, sports and events with no returned context and closed responses; finite boundary values and quoted exponents remain accepted. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (523 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent parsing safeguard does not establish live provider qualification or enable runtime wiring. The Odds API remains the sportsbook-price authority.
