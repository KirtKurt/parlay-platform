...
1. **Provider/context integration**
   ...
   - 2026-09-18 event-identity increment (item 1 only): event context now requires at least one valid `id`, `match_id` or `event_id`; all non-null aliases must agree after integer-to-string normalization. Missing IDs, booleans, negative integers, floats, structured values, empty/padded/control-containing strings and conflicting aliases reject the entire collection with `BBD_EVENT_ID_INVALID`. Nonnegative integer IDs (including zero) and opaque strings retain their identity; null aliases may fall back to a valid alias. Offline regressions cover invalid rows before and after valid rows, alias agreement and preservation of raw evidence. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (630 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This is a local fail-closed identity contract, not proof of live provider IDs or cross-provider identity mapping. The Odds API remains the sportsbook-price authority.
