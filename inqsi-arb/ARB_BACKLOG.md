...
1. **Provider/context integration**
   ...
   - 2026-09-26 response-size increment (item 1 only): successful BBD HTTP response bodies are limited to 1 MiB before UTF-8/JSON decoding. The transport reads at most the limit plus one byte and returns `BBD_RESPONSE_TOO_LARGE` with no context when exceeded, closing the response. Offline health, sports and events tests cover below/at/above-limit valid JSON and verify bounded reads and rejection before decoding. This conservative local limit is not a proven provider page-size contract; oversized collections remain unavailable pending qualified pagination. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (787 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (build succeeded; global SAM metadata writes blocked by the read-only home filesystem). Runtime wiring remains pending and The Odds API remains the sportsbook-price authority.
