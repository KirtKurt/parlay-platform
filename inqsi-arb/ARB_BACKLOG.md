...
1. **Provider/context integration**
   ...
   - 2026-09-12 collection-ambiguity increment (item 1 only): BBD collection parsing now requires exactly one recognized collection field at each supported envelope level. Competing fields fail closed with `BBD_COLLECTION_SCHEMA_INVALID`, including empty, malformed or error-bearing siblings, instead of selecting the first usable collection. Offline regressions cover health, sports and events, field-order independence, and preservation of all supported single-collection envelopes with metadata. Validation: `PYTHONDONTWRITEBYTECODE=1 python -m pytest -q -p no:cacheprovider inqsi-arb/tests` (555 passed); `SAM_CLI_TELEMETRY=0 PYTHONDONTWRITEBYTECODE=1 sam build --no-cached --template-file template.yaml` from `inqsi-arb` (exit 0, build succeeded; global SAM metadata writes blocked by the read-only home filesystem). This independent schema safeguard does not establish live provider qualification or enable runtime wiring; authenticated fixtures and entitlement proof remain unavailable. The Odds API remains the sportsbook-price authority.
