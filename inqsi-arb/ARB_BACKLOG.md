...
1. **Provider/context integration**
   ...
   - 2026-09-25 sports HTTP-status increment (item 1 only): sports discovery now discards context for non-200 responses before collection parsing, returning the HTTP status and `BBD_SPORTS_ENDPOINT_UNAVAILABLE_OR_UNENTITLED`. Previously, parseable non-200 responses could expose sports rows despite `ok: false`. Offline urllib regressions cover 201/202/203/204/205/206/299 with populated, empty, invalid-envelope and empty-body responses; 200 still preserves valid rows. Validation: all 804 ARB tests pass with bytecode/cache writes disabled; ARB SAM build succeeds (exit 0; global SAM metadata writes blocked by the read-only home filesystem). This independent status safeguard does not qualify live provider behavior or enable runtime wiring. The Odds API remains the sportsbook-price authority.
