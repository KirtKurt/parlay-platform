...
7. after marking the draft ready, ordinary exact-head PR CI is awaited again because `ready_for_review` may start new workflows. Approval requirements and failed checks still stop immediately; a queue timeout leaves the PR unmerged for review. After the wait, PR identity, ready state and main-drift checks are repeated immediately before merge;
...
The controller wrapper budget is 270 minutes within a 300-minute job. This allows 120 minutes for coding and independent validation/publication, two ordinary PR CI waits of up to 20 minutes each (before validation and after marking ready), two dispatch-discovery windows of 3 minutes each, 25 minutes of validation waiting and 50 minutes of deployment waiting, plus 29 minutes of wrapper overhead. The outer job reserves another 30 minutes for setup, credential cleanup and evidence upload. An explicit Actions approval requirement stops immediately. Validation permits 15 minutes of execution; deployment permits 15 minutes of tests plus 30 minutes of deployment. Queue/discovery delays and final timeouts remain explicit failures, not false successful releases. These are maximum failure deadlines, not a change to the one-action budget, promotion eligibility or default-disabled auto-promotion/deployment flags.
