...
@pytest.mark.parametrize("body", [
    ...
    b'{"data": [{"start_time": -Infinity}]}',
    b'{"data": [{"id": 1e400}]}',
    b'{"data": [{"start_time": -1E400}]}',
    b'{"data": [{"home": {"rating": 1.8e308}}]}',
    b'{"data": [], "meta": {"values": [1e9999]}}',
])
@pytest.mark.parametrize("operation", ["health", "sports", "events"])
...
def test_finite_json_numbers_and_quoted_exponents_remain_valid(transport):
    install, _, responses = transport
    install(200, body=b'{"data": [{"id": "1e400", "values": '
                      b'[1.7976931348623157e308, -1.7976931348623157e308, '
                      b'1.25, 1e-300, 0.0, 12345678901234567890]}]}')

    result = bbd_provider.events()

    assert result["ok"] is True
    assert result["events"][0]["bbd_event_id"] == "1e400"
    assert result["events"][0]["raw"]["values"] == [
        1.7976931348623157e308, -1.7976931348623157e308,
        1.25, 1e-300, 0.0, 12345678901234567890,
    ]
    assert responses[0].closed
