...
def test_issue_deduplication(monkeypatch):
    gh = FakeGH()
    c = controller(monkeypatch, gh)
    action = mod.Action("issue", "latency_acceptance_gate_failed", "latency")
    snap = make_snapshot(latency_conclusion="failure")
    first = c.execute([action], snap)
