...
def _baseball(*, jurisdiction: str, source: str, shortened_winner: str, shortened_spread: str, notes: str) -> None:
    register(Rule(
        book="betmgm", sport="baseball", market_family="winner", jurisdiction=jurisdiction,
        reviewed=True, version=REVIEW, source=source,
        settlement_profile="mlb_full_game_2way_action_v1" if "36h" not in shortened_winner else "betmgm_mlb_full_game_2way_action_36h_v1",
    ...
    for family in ("spreads", "totals"):
        register(Rule(
            ...
            reviewed=True, version=REVIEW, source=source,
            settlement_profile="mlb_full_game_9_or_8.5_v1" if "36h" not in shortened_spread else "betmgm_mlb_full_game_9_or_8.5_36h_v1",
            overtime=True, listed_pitcher=False,
