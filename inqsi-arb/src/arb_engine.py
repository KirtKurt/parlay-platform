...
The engine has no network or AWS dependencies. It only evaluates quote sets that
have already been normalized to the same canonical market identity.
"""
...
def _net_decimal(decimal_odds: float, commission_rate: float) -> float:
    """Reduce only winnings by exchange-style commission."""
...
def scan_market(
    *,
    ...
    rules_status: str = "provider_identity_only",
    context: Optional[Mapping[str, Any]] = None,
    commence_time: Optional[str] = None,
) -> Optional[Dict[str, Any]]:
    """Return an evaluated market, choosing the best compatible quote per outcome.

    A market is eligible to be labelled an arb only when all expected outcomes
    are present and the canonicalizer has not marked the rule identity unknown or
    incompatible. The calculation itself is deterministic and supports N outcomes.
    """
    ...
    for raw in quotes or []:
        ...
        if not outcome or not book:
            continue
        try:
            d = _quote_decimal(raw)
            net_d = _net_decimal(d, float(raw.get("commission_rate") or 0.0))
        ...
        candidate = {
            ...
            "decimal": d,
            "net_decimal": net_d,
            "commission_rate": float(raw.get("commission_rate") or 0.0),
        ...
        if previous is None or net_d > previous["net_decimal"]:
            best[outcome] = candidate

    if expected_set:
        missing = sorted(expected_set - set(best))
        extra = sorted(set(best) - expected_set)
        complete = not missing and not extra
    else:
        missing, extra = [], []
        complete = len(best) >= 2
        expected_set = set(best)

    if len(best) < 2:
        return None

    implied_sum = sum(1.0 / best[o]["net_decimal"] for o in sorted(expected_set) if o in best)
    rules_compatible = rules_status not in {"unknown", "incompatible"}
    is_arb = bool(complete and rules_compatible and implied_sum < 1.0)
    ...
    if complete and implied_sum > 0:
        ...
        rounded = {o: round(v, 2) for o, v in unrounded.items()}
        delta = round(bankroll - sum(rounded.values()), 2)
        if rounded and delta:
            largest = max(rounded, key=rounded.get)
            rounded[largest] = round(rounded[largest] + delta, 2)
        for outcome in sorted(expected_set):
            q = best[outcome]
            stake = rounded[outcome]
            payout = stake * q["net_decimal"]
            stake_rows.append({
                "outcome": outcome,
                "book": q["book"],
                "american": q["american"],
                "decimal": round(q["decimal"], 6),
                "net_decimal": round(q["net_decimal"], 6),
                "stake": stake,
                "payout_if_wins": round(payout, 2),
                "profit_if_wins": round(payout - bankroll, 2),
    ...
    return {
        ...
        "margin_pct": round(theoretical_return * 100.0, 4),
        "hold_pct": round((implied_sum - 1.0) * 100.0, 4),
        "bankroll": round(bankroll, 2),
        "legs": stake_rows,
        "minimum_payout": min_payout if is_arb else None,
        "minimum_profit": min_profit if is_arb else None,
        "n_quotes": valid_quotes,
        "n_books": len(seen_books),
        "outcomes": sorted(expected_set),
...
def scan_all(payload: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    for item in payload.get("events") or []:
        row = scan_market(
            ...
            rules_status=str(item.get("rules_status") or "provider_identity_only"),
            context=item.get("context") or {},
            commence_time=item.get("commence_time"),
        )
        if row is None:
            continue
        if row["validation"]["outcome_coverage"] != "complete" or row["validation"]["rules_status"] in {"unknown", "incompatible"}:
            rejected.append(row)
        elif row["arb"]:
            hits.append(row)
        else:
            near.append(row)
    hits.sort(key=lambda r: (r["minimum_profit"] or -10**9, r["margin_pct"]), reverse=True)
    near.sort(key=lambda r: r["sum_implied"])
    return {
        "ok": True,
        "places_bets": False,
        "bankroll": bankroll,
        "n_markets": len(hits) + len(near) + len(rejected),
        "n_arbs": len(hits),
