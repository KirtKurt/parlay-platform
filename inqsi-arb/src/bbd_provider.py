...
def _request(path: str, *, params: Optional[Dict[str, Any]] = None, timeout: int = 20,
    ...
    try:
        ...
        # Error bodies are untrusted and may echo credentials. Status alone is
        # sufficient evidence; do not consume the body, and always release it.
        try:
            if optional and exc.code in {401, 403, 404}:
                return int(exc.code), dict(exc.headers.items()), {}
            raise BBDError(f"BBD_HTTP_{exc.code}") from None
        finally:
            exc.close()
