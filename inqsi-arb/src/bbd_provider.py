...
def _items(payload: Any) -> List[Dict[str, Any]]:
    ...
    if payload.get("error") is not None:
        raise BBDError("BBD_COLLECTION_SCHEMA_INVALID")
    # Presence, not value type, determines ambiguity: a malformed sibling
    # must not be silently skipped in favor of a usable collection.
    keys = [key for key in ("data", "sports", "matches", "events", "results")
            if key in payload]
    if len(keys) != 1:
        raise BBDError("BBD_COLLECTION_SCHEMA_INVALID")
    ...
    if isinstance(value, dict):
        if value.get("error") is not None:
            raise BBDError("BBD_COLLECTION_SCHEMA_INVALID")
        nested_keys = [key for key in ("sports", "matches", "events", "results", "items")
                       if key in value]
