...
US_SPORTSBOOKS = {
    ...
    # Provider inventory is not licensing evidence. These keys are offered by
    # the upstream provider, but do not have a corroborated BOOK_STATES
    # footprint and therefore must not be presented as licensed sportsbooks.
    "betanysports": {"title": "BetAnything", "regions": ("us2",), "kind": "provider_only"},
    "courtside": {"title": "Courtside", "regions": ("us2",), "kind": "provider_only"},
    "rebet": {"title": "ReBet", "regions": ("us2",), "kind": "provider_only"},
}
...
def _reviewed_coverage(book: str) -> tuple[List[str], List[str]]:
    ...
    for rule in registry_rows():
        if str(rule.get("book") or "").strip().lower() != wanted or not rule.get("reviewed"):
            continue
        families.add(f"{rule.get('sport')}:{rule.get('market_family')}")
        jurisdictions.add(str(rule.get("jurisdiction") or "*"))
    return sorted(families), sorted(jurisdictions)
...
def catalog_summary() -> Dict[str, Any]:
    ...
    return {
        "ok": True,
        "complete": False,
        "catalog_scope": "static_metadata_overlay",
        "all_provider_books_claimed": False,
        "as_of": AS_OF,
        "count": len(rows),
        ...
        "n_provider_only": by_kind.get("provider_only", 0),
