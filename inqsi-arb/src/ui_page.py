    ...
    <div>
      <div class="panel">
        <div class="controls">
          ...
          <div><label>State books only</label><select id="licensed"><option value="false" selected>All books</option><option value="true">Licensed in selected state</option></select></div>
...
async function loadPacks(){try{const r=await fetch(api+'/v1/arb/packs');const j=await r.json();if(!r.ok)throw new Error(j.error||'packs unavailable');const select=el('jurisdiction');const selected=select.value;select.replaceChildren();for(const pack of (j.packs||[])){const option=document.createElement('option');option.value=String(pack.state||'');option.textContent=String(pack.name||pack.state||'');option.selected=option.value===selected;select.appendChild(option)}}catch(_){/* Keep the safe built-in fallback list. */}}
...
el('scanBtn').addEventListener('click',scan);el('allBtn').addEventListener('click',scanAllMarkets);loadPacks();health();refreshHistory();connectWs();
</script>
</body>
</html>'''
