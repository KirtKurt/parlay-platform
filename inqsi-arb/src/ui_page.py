  ...
  <div class="grid">
    <div>
      <div class="panel">
        ...
        <div class="books">
          <div class="bookhead">
            <div><b>Sportsbooks</b><div class="small" id="bookHint">All sportsbooks</div></div>
            <button type="button" class="secondary" id="clearBooks" style="padding:6px 10px">Show all</button>
          </div>
          <div id="bookList" class="bookgrid"><div class="small">Loading sportsbooks…</div></div>
          <label style="margin-top:8px">Other provider keys (comma-separated)</label><input id="bookKeys" placeholder="Any provider key missing from the catalog" style="width:100%">
        </div>
        <div class="small" style="margin-top:7px">Empty selection means every provider-returned sportsbook — US, offshore, DFS and exchanges stay on the math board. Verified is a house-rule stamp, not a hide-the-board switch. Integer spreads and totals include the push state.</div>
        <div class="metrics">
    ...
    <div>
      ...
      <div class="panel" style="margin-top:14px"><b>Safety posture</b><div class="small" style="margin-top:8px">You choose sportsbooks. A mathematical edge is not shown as verified unless the outcome set is exact and every participating sportsbook has an explicitly reviewed, compatible settlement profile. Unknown or materially different house rules remain unverified. Settlement is never copied across unread books.</div></div>
    </div>
  </div>
</div>
<script>
...
const KIND={us_licensed:'US sportsbooks',provider_only:'Provider-listed (license unverified)',offshore:'Offshore',dfs:'DFS',exchange:'Exchanges',international:'International',unknown:'Other'};
const el=id=>document.getElementById(id);
const esc=s=>String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
const fmt=n=>Number(n||0).toFixed(2);
function setDot(id,ok){el(id).className='dot '+(ok?'ok':'bad')}
function selectedBooks(){const checked=[...document.querySelectorAll('input[name=book]:checked')].map(i=>i.value);const manual=el('bookKeys').value.split(',').map(x=>x.trim().toLowerCase()).filter(Boolean);return [...new Set(checked.concat(manual))]}
function savedBooks(){try{const value=JSON.parse(localStorage.getItem('inqsi-arb-books')||'[]');return new Set(Array.isArray(value)?value:[])}catch(_){return new Set()}}
function restoreSavedBooks(){const saved=[...savedBooks()];el('bookKeys').value=saved.join(',');el('bookHint').textContent=saved.length?saved.length+' sportsbooks selected':'All sportsbooks'}
function persistBooks(){const selected=selectedBooks();try{localStorage.setItem('inqsi-arb-books',JSON.stringify(selected))}catch(_){}el('bookHint').textContent=selected.length?selected.length+' sportsbooks selected':'All sportsbooks'}
async function loadBooks(){try{const r=await fetch(api+'/v1/arb/books');const j=await r.json();const saved=savedBooks();const known=new Set((j.books||[]).map(b=>b.key));el('bookKeys').value=[...saved].filter(key=>!known.has(key)).join(',');const groups={};(j.books||[]).forEach(b=>{const k=b.kind||'unknown';(groups[k]=groups[k]||[]).push(b)});const order=['us_licensed','provider_only','offshore','dfs','exchange','international'];el('bookList').innerHTML=order.filter(k=>groups[k]).concat(Object.keys(groups).filter(k=>!order.includes(k))).map(k=>{const rows=groups[k].map(b=>{const detail=b.key+(b.regions?.length?' ['+b.regions.join(',')+']':'');return '<label class="book"><input type="checkbox" name="book" value="'+esc(b.key)+'"'+(saved.has(b.key)?' checked':'')+'><span>'+esc(b.title)+' · '+esc(detail)+'</span></label>'}).join('');return '<div class="bookgroup"><div class="small">'+esc(KIND[k]||k)+'</div>'+rows+'</div>'}).join('')||'<div class="small">No sportsbooks in catalog.</div>';el('bookList').querySelectorAll('input').forEach(i=>i.addEventListener('change',persistBooks));persistBooks()}catch(e){el('bookList').innerHTML='<div class="small">Book list unavailable. Enter provider keys directly below or scan all sportsbooks.</div>'}}
...
el('scanBtn').addEventListener('click',scan);el('allBtn').addEventListener('click',scanAllMarkets);el('bookKeys').addEventListener('change',persistBooks);el('clearBooks').addEventListener('click',()=>{document.querySelectorAll('input[name=book]').forEach(i=>i.checked=false);el('bookKeys').value='';persistBooks()});restoreSavedBooks();health();loadBooks();refreshHistory();connectWs();
</script>
</body>
</html>'''
