...
const KIND={us_licensed:'US sportsbooks',offshore:'Offshore',dfs:'DFS',exchange:'Exchanges',international:'International',unknown:'Other'};
...
function persistBooks(){const selected=selectedBooks();localStorage.setItem('inqsi-arb-books',JSON.stringify(selected));el('bookHint').textContent=selected.length?selected.length+' sportsbooks selected':'All sportsbooks'}
async function loadBooks(){try{const r=await fetch(api+'/v1/arb/books');const j=await r.json();const saved=new Set(JSON.parse(localStorage.getItem('inqsi-arb-books')||'[]'));const groups={};(j.books||[]).forEach(b=>{const k=b.kind||'unknown';(groups[k]=groups[k]||[]).push(b)});const order=['us_licensed','offshore','dfs','exchange','international'];el('bookList').innerHTML=order.filter(k=>groups[k]).concat(Object.keys(groups).filter(k=>!order.includes(k))).map(k=>{const rows=groups[k].map(b=>'<label class="book"><input type="checkbox" name="book" value="'+esc(b.key)+'"'+(saved.has(b.key)?' checked':'')+'><span>'+esc(b.title)+'</span></label>').join('');return '<div class="bookgroup"><div class="small">'+esc(KIND[k]||k)+'</div>'+rows+'</div>'}).join('')||'<div class="small">No sportsbooks in catalog.</div>';el('bookList').querySelectorAll('input').forEach(i=>i.addEventListener('change',persistBooks));persistBooks()}catch(e){el('bookList').innerHTML='<div class="small">Book list unavailable. Scan still includes every provider-returned sportsbook.</div>'}}
...
el('scanBtn').addEventListener('click',scan);el('allBtn').addEventListener('click',scanAllMarkets);el('clearBooks').addEventListener('click',()=>{document.querySelectorAll('input[name=book]').forEach(i=>i.checked=false);persistBooks()});health();loadBooks();refreshHistory();connectWs();
</script>
</body>
</html>'''
